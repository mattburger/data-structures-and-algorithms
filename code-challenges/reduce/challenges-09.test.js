...
const countNumberOfElements = (arr) => arr.reduce((total) => total + 1, 0 );
...
const returnNames = (arr) => {
  ...
  arr.reduce((previousValue, currentValue) => {
    finalRes.push(currentValue.name);
  }, arr[0] );
  return finalRes;
};
/* ------------------------------------------------------------------------------------------------
...
const reversedString = (arr) => arr.split('').reduce((prev, curr) => curr + prev,'');
...
const countNumberOfChildren = (arr) => {
  ...
  arr.reduce( (prev, curr) => {
    if( !(typeof(prev) === 'undefined')){
      total += prev.children.length;
      total += curr.children.length;
    }
    else if(!(typeof(curr.children)==='undefined')){
      total += curr.children.length;
    }
  }), arr[0];
  return total;

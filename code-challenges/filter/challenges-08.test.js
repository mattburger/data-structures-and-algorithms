...
const oddValues = (arr) => arr.filter(e => e % 2 !== 0);
...
const filterStringsWithVowels = (arr) => arr.filter( e => /[aeiou]/.test(e));
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter( e => !forbiddenValues.includes(e));
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter(e => e.baseStat > minBaseStat);
...
//   return arr3;
// };
const getStatName = (arr, minBaseStat) => arr.filter(e => e.baseStat > minBaseStat).map(e =>e.stat.name);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6

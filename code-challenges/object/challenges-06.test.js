...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(element => {
    sizes.push({house: element.house, members: 1});
    sizes[sizes.length-1].members += element.children.length;
    if(!(element.spouse === null)){
      sizes[sizes.length-1].members++;
    }
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  const survivors = [];
  arr.forEach(element => {
    survivors.push({house: element.house, members: 1});
    survivors[survivors.length - 1].members += element.children.length;
    let counter = 0;
    deceasedSpouses.forEach(e =>{
      if(element.spouse === e || element.spouse === null){
        ++counter;
      }
    });
    if(counter < 1){
      survivors[survivors.length - 1].members++;
    }
  });
  return survivors;
};
